...
test('dark outfit preset switches visible dark parts and clothing parameters', () => {
  ...
  assert.equal(dark.parts.Part28, 0)
  assert.equal(normal.parts.Part20, 0)
  assert.equal(normal.parts.Part28, 1)

  assert.equal(dark.params.Param23, 1)
  assert.equal(dark.params.Param33, 1)
  assert.equal(dark.params.Param34, 1)
  assert.equal(dark.params.Param35, 1)
  assert.equal(dark.params.Param36, 1)
  assert.equal(dark.params.Param37, 1)
})
